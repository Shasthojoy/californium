 ...
 * 
 * Analyzing usage of {@link KeyManager} and {@link TrustManager}.
 */
public class SslContextLoggingUtil {
	 ...
	 */
	private static final Level KEY_MANAGER_DETAIL_LEVEL = Level.FINE;
	/**
	 * Logging level for trust manager.
	 * 
	 * @see #logging(TrustManager[])
	 */
	private static final Level TRUST_MANAGER_DETAIL_LEVEL = Level.FINE;
	 ...
	 */
	private static final AtomicInteger LOGGING_ID = new AtomicInteger();
		...
		if (LOG.isLoggable(KEY_MANAGER_DETAIL_LEVEL)) {
			for (int index = 0; index < managers.length; ++index) {
				if (managers[index] instanceof X509ExtendedKeyManager) {
					managers[index] = new LoggingX509ExtendedKeyManager((X509ExtendedKeyManager) managers[index],
							label);
				}
			}
		}
	...
	private static class LoggingX509ExtendedKeyManager extends X509ExtendedKeyManager {
		 ...
		 *            {@link SslContextLoggingUtil#LOGGING_ID} is used.
		 */
		public LoggingX509ExtendedKeyManager(X509ExtendedKeyManager manager, String label) {
			this.manager = manager;
			String id = Integer.toString(LOGGING_ID.incrementAndGet());
			if (null == label || label.isEmpty()) {
				this.id = id;
			} else {
				this.id = label + "-" + id;
			}
			LOG.log(KEY_MANAGER_DETAIL_LEVEL, "LoggingX509ExtendedKeyManager[ID {0}] {1}",
					new Object[] { this.id, manager });
		}

		@Override
		public String chooseEngineClientAlias(String[] keyType, Principal[] issuers, SSLEngine engine) {
			String alias = manager.chooseEngineClientAlias(keyType, issuers, engine);
			LOG.log(KEY_MANAGER_DETAIL_LEVEL, "[ID {0}] key type {1}, issuer {2}, {3} => {4}",
					new Object[] { id, list(keyType), list(issuers), engine, alias });
			return alias;
		}
		...
		public String chooseEngineServerAlias(String keyType, Principal[] issuers, SSLEngine engine) {
			...
			LOG.log(KEY_MANAGER_DETAIL_LEVEL, "[ID {0}] key type {1}, issuer {2}, {3} => {4}",
					new Object[] { id, keyType, list(issuers), engine, alias });
			return alias;
		}
		...
		public String chooseClientAlias(String[] keyType, Principal[] issuers, Socket socket) {
			...
			LOG.log(KEY_MANAGER_DETAIL_LEVEL, "[ID {0}] key type {1}, issuer {2}, {3} => {4}",
					new Object[] { id, list(keyType), list(issuers), socket, alias });
			return alias;
		}
		...
		public String chooseServerAlias(String keyType, Principal[] issuers, Socket socket) {
			...
			LOG.log(KEY_MANAGER_DETAIL_LEVEL, "[ID {0}] key type {1}, issuer {2}, {3} => {4}",
					new Object[] { id, keyType, list(issuers), socket, alias });
			return alias;
		}
		...
		public String[] getClientAliases(String keyType, Principal[] issuers) {
			...
			LOG.log(KEY_MANAGER_DETAIL_LEVEL, "[ID {0}] key type {1}, issuer {2} => {3}",
					new Object[] { id, keyType, list(issuers), list(alias) });
			return alias;
		}
	...
	private static class LoggingX509ExtendedTrustManager extends X509ExtendedTrustManager {
		...
		public LoggingX509ExtendedTrustManager(X509ExtendedTrustManager manager, String label) {
			...
			LOG.log(TRUST_MANAGER_DETAIL_LEVEL, "LoggingX509ExtendedTrustManager[ID {0}] {1}",
					new Object[] { this.id, manager });
		}
		...
		public void checkClientTrusted(X509Certificate[] chain, String authType, Socket socket)
			...
			LOG.log(TRUST_MANAGER_DETAIL_LEVEL, "[ID {0}] {1} {2} {3}",
					new Object[] { id, list(chain), authType, socket });
			try {
		...
		public void checkClientTrusted(X509Certificate[] chain, String authType, SSLEngine engine)
				throws CertificateException {
			LOG.log(TRUST_MANAGER_DETAIL_LEVEL, "[ID {0}] {1} {2} {3}",
					new Object[] { id, list(chain), authType, engine });
			try {
